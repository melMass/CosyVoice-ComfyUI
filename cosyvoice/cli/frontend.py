...
def text_normalize(text):
    ...
    _txt = "".join(sentences)
    # Replace all characters except Chinese ones
    # _txt = re.sub(
    #     r"[^\u4e00-\u9fa5，。！？、]+", "", _txt
    # )
...
def remove_chinese_punctuation(text):
    """
    Remove Chinese punctuation marks and replace them with commas.
     :param text:
     :return:
    """
    ...
    # remove commas at the beginning and end of a text string.
    text = re.sub(r"^，|，$", "", text)
    return text
...
def normalize_zh(text):
    ...
    if text.startswith("<|zh|>"):
        n_text = process_ddd(text_normalize(remove_chinese_punctuation(text[6:])))
        return "<|zh|>" + n_text
    if (
        "<laughter>" in text
        or "<strong>" in text
...
def process_ddd(text):
    """
    This function processes the use of the characters '地' and '得'. It replaces them with '的'.
    Rationale: The primary usage of '地' and '得' is before or after verbs and adjectives. However, this method does not strictly replace according to grammar rules because these characters are often used incorrectly. Additionally, the accuracy of jieba (a Chinese word segmentation tool) can cause some instances to be missed. For example: Xiao Hongmao was puzzled.
