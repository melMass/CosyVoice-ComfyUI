...
class CosyVoiceModel:
    ...
    def token2wav(
        self, token, prompt_token, prompt_feat, embedding, uuid, finalize=False
    ):
        with self.flow_hift_context:
            ...
            if self.mel_overlap_dict[uuid] is not None:
                tts_mel = fade_in_out(
                    tts_mel, self.mel_overlap_dict[uuid], self.mel_window
                )
            # append hift cache
        ...
        **kwargs,
        ...
        with self.lock:
            (
                ...
                self.llm_end_dict[this_uuid],
                self.mel_overlap_dict[this_uuid],
                self.hift_cache_dict[this_uuid],
            ) = [], False, None, None
        p = threading.Thread(
            target=self.llm_job,
